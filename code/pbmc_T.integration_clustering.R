...
out <- here("data","SCEs","merge",paste0(celltype,".SCT.obj.list.RData"))
s <- here("data","SCEs","merge",paste0(celltype,".preprocessed.merged.SEU.qs"))

if(!file.exists(out)) {
  seu <- qread(here("data","SCEs","manual.annot",paste0(celltype,".SEU.qs")),nthreads=16)
...
if(!file.exists(out)) {
  ...
  # PrepSCTFindMarker
  plan("multicore", workers=length(levels(factor(seu$init.clust))))
