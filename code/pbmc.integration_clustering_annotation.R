...
if(!file.exists(out)) {
  seu <- qread(here("data","SCEs","integration",paste0(celltype,".preprocessed.merged.integrated.SEU.qs")), nthreads=16)
  DefaultAssay(seu) <- "SCT"
...
# Annotation - Yoshida
# 3 Reference-based annotation using Yoshida 2021 dataset
## 3.1 Prepare Yoshida reference data
# Source: https://cellxgene.cziscience.com/collections/03f821b4-87be-4ff4-b65a-b5fc00061da7
out <- "/group/canc2/anson/reference/annotation/Yoshida_2021/yoshida_2021.processed_v2.SEU.qs"
seu.ref <- qread(out,nthreads=16)
...
# Find anchors
if (!file.exists("/group/canc2/anson/reference/annotation/Yoshida_2021/anchors_Yoshida_250429.qs")) {
  anchors <- FindTransferAnchors(reference=seu.ref,
                                 ...
                                 query.assay="SCT",
                                 reference.assay="SCT")
  qsave(anchors, file="/group/canc2/anson/reference/annotation/Yoshida_2021/anchors_Yoshida_250429.qs", nthreads=16)
} else {
  anchors <- qread("/group/canc2/anson/reference/annotation/Yoshida_2021/anchors_Yoshida_250429.qs", nthreads=16)
}
...
seu.query <- MapQuery(anchorset = anchors,
                      reference = seu.ref,
                      query = seu.query,
                      refdata=list(annotation_broad="annotation_broad",
                                   annotation_detailed="annotation_detailed"),
                      reference.reduction = "pca",
                      reduction.model = "umap")
out <- here("data","SCEs","annotation","pbmc.preprocessed.merged.integrated.clustered.Yoshida.SEU.qs")
if (!file.exists(out)) {qsave(seu.query, file=out, nthreads=16)}
print("Yoshida annotation done.")

# Annotation - Stephenson
out <- "/group/canc2/anson/reference/annotation/Stephenson_2021/stephenson_2021.processed.SEU.qs"
seu.ref <- qread(out,nthreads=16)
DefaultAssay(seu.ref) <- "SCT"

# Find anchors
if (!file.exists("/group/canc2/anson/reference/annotation/Stephenson_2021/anchors_Stephenson_250429.qs")) {
  anchors <- FindTransferAnchors(reference=seu.ref, 
                                 query=seu.query,
                                 normalization.method="SCT",
                                 reference.reduction="pca",
                                 ...
                                 query.assay="SCT",
                                 reference.assay="SCT")
  qsave(anchors, file="/group/canc2/anson/reference/annotation/Stephenson_2021/anchors_Stephenson_250429.qs", nthreads=16)
} else {
  anchors <- qread("/group/canc2/anson/reference/annotation/Stephenson_2021/anchors_Stephenson_250429.qs", nthreads=16)
}
...
seu.query <- MapQuery(anchorset = anchors, 
                      reference = seu.ref, 
                      query = seu.query,
                      refdata=list(initial_clustering="initial_clustering",
                                   author_cell_type="author_cell_type"), 
                      reference.reduction = "pca", 
                      reduction.model = "umap")
out <- here("data","SCEs","annotation","pbmc.preprocessed.merged.integrated.clustered.Yoshida.Stephenson.SEU.qs")

if (!file.exists(out)) {qsave(seu.query, file=out, nthreads=16)}
print("Stephenson annotation done. Please proceed to HLCA azimuth annotation.")
